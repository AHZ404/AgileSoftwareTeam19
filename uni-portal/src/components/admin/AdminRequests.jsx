...
const AdminRequests = () => {
  ...
  return (
    <div id="all-requests-section" className="content-section">
      <h2 style={{ color: 'var(--primary)', marginBottom: '20px' }}>All Course Requests</h2>
          ...
          requests.map(request => {
            ...
            return (
              <div key={request.id} className="request-item" style={{ 
                ...
                <div className="request-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  <div className="request-info">
                    <h4 style={{ margin: '0 0 5px 0', color: 'var(--primary)', fontSize: '1.1rem' }}>
                        {request.courseId}: {course.title}
                    </h4>
                    <div className="request-meta" style={{ color: '#6c757d', fontSize: '0.9rem' }}>
                        Student: {student.firstName} {student.lastName} ({request.studentId}) | Submitted: {request.dateSubmitted}
                    </div>
                  </div>
                  <span className={`request-status status-${request.status}`} style={{
                      backgroundColor: request.status === 'pending' ? '#fff3cd' : request.status === 'approved' ? '#d1edff' : '#f8d7da',
                      color: request.status === 'pending' ? '#856404' : request.status === 'approved' ? '#0d6efd' : '#842029',
                      padding: '5px 10px',
                      borderRadius: '15px',
                      fontSize: '0.75rem',
                      fontWeight: '700',
                ...
                <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '5px', margin: '15px 0', fontSize: '0.95rem' }}>
