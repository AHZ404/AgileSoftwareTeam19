...
const StudentClassrooms = ({ user }) => {
  ...
  const handleSearch = () => {
    const rooms = universityDB.getAvailableClassrooms(
      filters.date,
      filters.start,
      filters.end
    );
  ...
  const handleBookingSubmit = () => {
    ...
    try {
      ...
      alert(
        user.role === "advisor" ? "Booking Approved!" : "Booking Requested!"
      );
      setModalOpen(false);
      loadBookings();
      handleSearch();
    ...
    <div>
        ...
        {availableRooms.map((room) => (
          <div key={room.id} className="course-card">
            ...
            <p>
              ...
              {room.features ? room.features.join(", ") : "N/A"}
            </p>
      ...
      <div className="widget large" style={{ marginTop: "30px" }}>
        ...
        <div className="requests-grid">
          {myBookings.length === 0 ? (
            <p className="placeholder-text">No bookings found.</p>
          ) : (
            myBookings.map((b) => {
              const room = universityDB
                .getAllClassrooms()
                .find((r) => r.id === b.classroomId) || { name: "Unknown" };
              return (
                <div key={b.id} className="request-item">
                  <div className="request-header">
                    <div className="request-info">
                      <h4>
                        {room.name} ({b.classroomId})
                      </h4>
                      <div className="request-meta">
                        {b.date} | {b.startTime} - {b.endTime}
                      </div>
                    </div>
                    <span className={`request-status status-${b.status}`}>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <p className="modal-info-text">
                Date: {filters.date} <br /> Time: {filters.start} -{" "}
                {filters.end}
              </p>
