  ...
  return (
    <div>
          ...
          return (
            <div key={course.id} className="course-card" style={{borderTopColor: course.color || 'var(--primary)'}}>
               ...
               <div className="course-header">
                 <h4 style={{fontSize: '1.2rem', margin: 0}}>{course.id}: {course.title}</h4>
               ...
               <div style={{marginTop: '10px', color: '#444'}}>
                   <p style={{margin: '4px 0'}}><strong>Instructor:</strong> {instructor.firstName} {instructor.lastName}</p>
                   <p style={{margin: '4px 0'}}><strong>Schedule:</strong> {course.schedule} | {course.location}</p>
               </div>
               ...
               <div className="course-footer" style={{borderTop: '1px solid #eee', paddingTop: '15px', textAlign: 'right'}}>
                 <button className="btn btn-register" onClick={() => handleRequestClick(course)} style={{
                     backgroundColor: '#f72585', // Pink color matching image
                     color: 'white',
                     border: 'none',
                     padding: '8px 16px',
                     borderRadius: '4px',
      ...
      <div className="requests-grid">
        {myRequests.length === 0 ? <p className="placeholder-text">No requests found.</p> : myRequests.map(r => (
           <div key={r.id} className="request-item" style={{
               borderLeftColor: r.status === 'pending' ? 'var(--warning)' : r.status === 'approved' ? 'var(--success)' : 'var(--danger)',
               marginTop: '10px'
           }}>
             <div className="request-header">
                <h4>{r.courseId}</h4>
                <span className={`request-status status-${r.status}`}>{r.status.toUpperCase()}</span>
             </div>
             <p style={{fontSize:'0.9rem', color:'#666'}}>Reason: {r.reason}</p>
           </div>
        ))}
      </div>
        ...
        <div className="modal" style={{display: 'flex'}}>
          <div className="modal-content">
            ...
            <div className="modal-body">
                <p className="modal-info-text">You are about to request registration for <strong>{selectedCourse?.id}</strong>.</p>
